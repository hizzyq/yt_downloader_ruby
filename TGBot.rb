...
require "dotenv/load"
require "logger"
require "sinatra" # Добавлен Sinatra для API
require "json"
...
# Поток для обработки событий от воркера через Redis
Thread.new do
...
# Поток для Telegram Бота (Long-polling)
Thread.new do
  loop do
    begin
      ...
      logger.error("Telegram Bot Error: #{e.message}. Перезапуск через 5 секунд...")
      sleep 5
    end
  end
end

# Основной поток заблокируется здесь сервером Sinatra
...
get '/health' do
  content_type :json
  { status: 'ok', message: 'Bot and API are running' }.to_json
end

# Пример эндпоинта. Вы можете расширить его по своему желанию.
post '/api/task' do
  content_type :json
  ...
  type = request_payload['type'] || 'preview'
  format = request_payload['format']

  if chat_id && url
    VideoWorker.perform_async(chat_id, url, type, format)
    { status: 'success', message: 'Task added to worker' }.to_json
  else
    status 400
    { status: 'error', message: 'chat_id and url are required' }.to_json
  end
end